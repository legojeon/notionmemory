"""SessionStart 온보딩 넛지 — 코어(PAT/memory) 미설정이고 아직 제안 안
...
def _probe(connected, mem_bound, indexed=False):
    return {
...
def _patch_probe(monkeypatch, connected, mem_bound, indexed=False):
    from notionmemory.core import status
    monkeypatch.setattr(
        status, "probe",
        lambda config, verify=True: _probe(connected, mem_bound, indexed))


def test_unoffered_pat_missing_offers_onboard(monkeypatch):
    _patch_probe(monkeypatch, False, False)
...
def test_core_all_set_is_silent_and_no_marker(monkeypatch):
    from notionmemory.core import paths
    from notionmemory.core.config import Config
    # library 미색인이어도 코어(PAT/memory)가 되면 onboard 제안 안 함
    _patch_probe(monkeypatch, True, True, indexed=False)
    assert session_start.onboarding_injection() == ""
