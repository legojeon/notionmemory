<p align="center"><a href="EXAMPLES.md">English</a> | <b>한국어</b></p>

# Examples

아래 예시는 어느 것도 notionmemory에 **기본 탑재된 기능이 아닙니다.** 각각은 **여러분 자신의**
Notion 데이터베이스·페이지이고, **templates** 스킬로 한 번 등록해 둔 것입니다 — 그리고 더
정교한 예시에는 에이전트가 어떻게 저작할지 알려주는 **attached prompt**가 붙어 있습니다.
등록해 두면 그 뒤로는 그냥 말만 하면 되고, 에이전트가 그 스펙을 따릅니다.

각 카드는 두 부분으로 되어 있습니다: **base template**(여러분 워크스페이스로 복제해 갈 수 있는
공개 Notion 원본 템플릿)과, 쓰인 경우 **attached prompt**(Notion 페이지가 아니라 여러분의
config에 사는 notionmemory 레이어)입니다. 아래 프롬프트는 작성자의 실제 한국어 프롬프트이며,
자유롭게 고쳐 쓰면 됩니다.

> 스크린샷은 아직 자리표시자입니다 — 작성자가 나중에 추가합니다.
...
> *"이 논문 읽을 자료에 추가해줘 — DOI 10.1088/…, 태그 exoplanets, 상태 To-read."*

에이전트가 여러분의 트래커 DB에 **속성 이름 그대로** 행을 씁니다 — 상태·태그·DOI — 그리고
나중에 *"아직 안 읽은 것만 보여줘"* 하면 같은 속성으로 필터링해 돌려줍니다. templates의 가장
기본 동작입니다: 임의의 Notion DB를 등록하고, 스키마 배선 없이 평범한 말로 CRUD 합니다.

- **Base template:** [Research Paper Tracker](https://www.notion.com/templates/research-paper-tracker)
- **Attached prompt:** 없음 — 속성 CRUD만.
...
> *"아이디어 하나 기록해줘: 셸 히스토리를 런북으로 바꿔주는 CLI."*

한 줄짜리 제목만 저장하는 게 아니라, 에이전트가 템플릿 프롬프트를 따라 페이지 본문을 네 섹션으로
채웁니다 — **해결할 문제와 대상 사용자**, 유사 도구·재사용 가능한 라이브러리를 실제로 찾는
**시장 조사**, 비판적인 **현실성·리스크** 검토(안 될 이유를 최소 하나는 반드시 적도록 지시됨),
그리고 **다음 단계** — 그런 뒤 Feasibility / Impact / Effort를 조사 결과에 맞춰 정리합니다.
*"그냥 빨리 담아만 둬"* 라고 하면 조사는 건너뛰고 행만 기록합니다.

- **Base template:** [Idea Bank Database](https://www.notion.com/templates/idea-bank-database)
- **Attached prompt:** ↓

<details>
<summary>Attached prompt (작성자 원본, 한국어) — 펼쳐 보기</summary>
...
> *"내 사이드 프로젝트 포트폴리오에 추가해줘."*

에이전트가 **실제 저장소**를 조사합니다 — 버전이 포함된 의존성, 디렉터리 구조, 개발 흐름과
fix/refactor 커밋을 담은 `git log`, CI·Docker 파일, 스크린샷 폴더 — 찾은 것을 먼저 보고하고,
코드로 알 수 없는 몇 가지(동기, 실제 성과 수치, 배포 여부)를 당신에게 물은 뒤,
**근거 기반** 프로젝트 카드를 실제 코드 스니펫과 크롭한 그림과 함께 작성합니다. 코드·커밋으로
확인되지 않은 것은 절대 쓰지 못하도록 명시돼 있어, 템플릿을 메우려는 추측이 없습니다.
...
*블루프린트 — 프롬프트 전용, 과목마다 재사용.*

> *"이 강의 PDF를 노트로 정리해줘."*

프롬프트 전용 **블루프린트**가 매번 새로운, 구조화된 노트를 찍어냅니다. PDF·슬라이드는
여러분이 읽고, 에이전트는 **원본과 같은 언어로** 개인 지식 노트를 씁니다 — 슬라이드 순서가
아니라 개념 중심으로 재편성하고, 핵심 용어는 첫 등장에 병기하고, 중요한 그림은 그것을 설명하는
문단 바로 옆에 크롭해 넣으며, 원본에 없는 것은 지어내지 않습니다. 프롬프트 전용이라 복제할
페이지는 없고, 아래 프롬프트를 `lecture-note` 블루프린트에 붙여 재현합니다.

- **Base template:** 없음 — 프롬프트 전용 블루프린트(Notion 페이지 없음).
- **Attached prompt:** ↓
...
페이지나 DB를 등록하고 — *"이 Notion 페이지를 템플릿으로 등록해줘"* (URL 붙여넣기) — 에이전트가
특정 방식으로 저작하길 원하면 **프롬프트를 붙이면** 됩니다: 설정 대시보드에서 지정하거나, 그냥
에이전트에게 프롬프트에 뭐라고 쓸지 말하면 됩니다. 그게 설정의 전부입니다. 전체 표면은
[templates 스킬](README.ko.md#agents)을 참고하세요.
