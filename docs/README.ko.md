...
바꿔줍니다. 장기 기억, 내가 만드는 페이지 유형(템플릿), 내용 검색을 **설치형 스킬**로 제공하고, 세션 훅이 필요한
...
**내 페이지 유형**
> *"내가 주간 보고서 만드는 방식이 이건데, 기억해줘."* (내가 만든 페이지 붙여넣기)
> *"이번 주에 한 일로 그 형태로 보고서 초안 써줘."*
...
- **templates** — 내가 만드는 페이지 유형을 기억(내가 만든 Notion 페이지·DB를 가리키면)해 에이전트가 같은 형태로 새 항목을 저작
...
- 이 정리 패스는 **저절로도 돕니다** — 세션 종료/시작 훅이 백그라운드로 스폰합니다 — 그리고
  세션 트랜스크립트에서 곧바로 새 메모리를 발굴해 Active 상태로 실제 Strength까지 매겨 저장합니다.
  트랜스크립트는 로컬에서 다시 읽히고, 그 발췌가 **설정된 agent 런타임**(`integrations.agent.backend`
