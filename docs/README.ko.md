...
**notionmemory**는 여러분의 Notion을 코딩 에이전트(Claude Code, Codex, Kimi Code, pi, opencode)가
함께 쓰는 장기 기억으로 바꿔줍니다. 장기 기억, **내가 만드는 페이지 유형**(내가 페이지 만드는 방식을
보여주면 에이전트가 같은 식으로 새 페이지를 씁니다), 내용 검색을 **설치형 스킬**로 제공하고,
세션 훅이 필요한 맥락을 알아서 띄워줍니다. 데이터는 전부 **여러분의 Notion**에 그대로 쌓입니다 —
별도의 DB도 서버도 없습니다.

명령어를 직접 입력할 필요는 없습니다. 에이전트에게 **평소처럼 말만 하세요** — "이거 기억해둬",
"이거 어디에 정리했더라?", "내가 만드는 이런 페이지 더 만들어줘" — 그러면 에이전트가
...
**Kimi Code / pi / opencode (각각 한 명령)**

```bash
pipx install notionmemory
notionmemory install --kimi        # 또는: --pi   /   --opencode
```

> 이 셋은 마켓플레이스 단계가 없습니다 — 한 명령이 스킬과 각 에이전트의 연동을 함께 설치합니다
> (pi·opencode는 작은 플러그인 번들, Kimi Code는 `config.toml` 훅). `notionmemory teardown`으로 전부 제거됩니다.
...
**기억하고 불러오기**
> *"인증을 JWT refresh-token 회전으로 바꿨다고 기억해둬."*
> *"레이트 리밋은 어떻게 처리하기로 했지?"*

**Notion 검색**
> *"배포 런북 어디에 적어놨더라?"*

**Notion에 작성하기**
> *"내가 주간 보고서 쓰는 방식이야 — 기억해두고 톤은 간결하게."* (내가 만든 페이지 붙여넣기)
> *"이번 주 한 일로 그 형태로 초안 써줘. 이 PDF의 그림 3도 넣고."*
...
**BM25** 랭킹을 Notion에 라이브 검증하고, 그 위에서 에이전트가 의미 판단을 합니다. 솔직한
주의점: 두 코퍼스 모두 소규모(수작업 채점 15+6문항 — LongMemEval은 타입별 1문항 표본이지
전체 500문항이 아님)이고, recall 한 번의 지연은 라이브 Notion 왕복(~1초)이며, 원시 트랜스크립트
코퍼스는 본래 용도를 벗어난 스트레스 테스트입니다 — notionmemory가 다루도록 설계된 건 증류된
메모리이고, 그쪽 점수는 최소한 같거나 더 좋습니다.
...
시점의 에이전트 의미 판단으로 메워집니다. 솔직한 주의점: 우리 행은 배포된 랭킹 코드의
...
- **templates** — 에이전트에게 *내가 페이지 만드는 방식*을 가르칩니다. 내가 만든 Notion 페이지·DB를 가리키고 프롬프트를 붙이면 — 구조·톤·조사해서 채울 섹션, 혹은 그냥 나에게 남기는 메모까지 — 에이전트가 정해진 템플릿이 아니라 **내 워크플로우**대로 새 항목을 Notion에 작성
...
- 모든 메모리에는 **Strength(1–10)** 중요도 점수가 붙습니다. 이 점수로 회수 순위를 정하고, 세션
  시작 때 무엇을 보여줄지 결정합니다.
- **정리 패스**가 초안을 검토합니다: 요약·정제하고, 실제 Strength를 매기고, 가치 없는 건
  버리고(→ *Forgotten*), 중복은 병합하고(→ *Superseded*), 프로젝트별 **요약(brief)**을 갱신해 —
  DB가 아무거나 쌓인 더미가 아니라 잘 정리된 상태로 유지됩니다. 세션 트랜스크립트에서 새 메모리를
  발굴하기도 합니다. 이 패스는 **자동으로** 백그라운드에서 돌거나(세션 종료/시작 훅), 직접
  `notionmemory memory consolidate`로 실행할 수 있습니다.
- 요약에는 **설정된 agent CLI**로 세션 발췌를 보내므로 백그라운드에서 **그 계정의 사용량을 씁니다** —
  그리고 **Notion에는 증류된 메모리만** 들어갑니다. 자동 패스를 끄려면
  `~/.config/notionmemory/config.yaml`에서 `skills.memory.consolidate_mode: nudge`로 바꾸세요
  (수동/cron만). 백엔드·모델(`integrations.agent.backend` / `.model`)도 같은 파일에 있으니 직접
  고치거나 에이전트에게 바꿔 달라고 하면 됩니다.
...
두 가지 이유 — *어디서* 도는가, 그리고 *무엇을* 할 수 있는가.
...
- 같은 CLI가 **헤드리스 환경이나 cron**에서도 돌아갑니다(예: 정리 패스). MCP를 지원하는 채팅
  안에서만 쓰는 게 아닙니다.
- 스킬 묶음 + CLI 하나로 **지원하는 모든 에이전트에서 똑같이** 동작합니다. MCP는 에이전트마다 지원
  범위와 동작이 다릅니다.
- **구성 요소가 적습니다.** 상시 서버가 없고, 토큰은 OS 키링에 있고, 로컬에 남는 건 가벼운 검색
  색인뿐입니다.
- **텍스트만이 아니라 실제 콘텐츠를 씁니다.** REST API를 직접 쓰기에 마크다운을 Notion 블록으로
  바꾸고, 페이지·데이터베이스를 만들고 고치며, **Notion Direct Upload로 이미지를 페이지에 바로
  올립니다** — 그래서 에이전트가 PDF에서 크롭한 그림을 내 노트에 그대로 넣을 수 있습니다. 읽기와
  크롭은 에이전트 자신의 도구가 하고, Notion 쓰기는 notionmemory 몫입니다 — 읽기 위주의 MCP
  흐름으로는 안 되는 부분입니다.

**MCP가 더 맞는 경우:** 셸이 없는 클라이언트 — Claude Desktop, Gemini, 웹·모바일 챗 — 에서
쓰거나, 아무것도 안 깔고 어느 기기에서나 닿고 싶다면 그건 MCP의 영역입니다. MCP 서버 하나면
에이전트별 코드 없이 모든 MCP 클라이언트에서 돌죠. notionmemory는 그 도달 범위를 내주는 대신
CLI가 가장 잘하는 것 — 세션 경계에서의 자동 캡처, 헤드리스 정리, 페이지에 실제 콘텐츠 쓰기 — 을
택했습니다. 다른 일에 맞는 다른 도구인 셈입니다.
